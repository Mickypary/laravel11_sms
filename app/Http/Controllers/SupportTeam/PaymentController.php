...
class PaymentController extends Controller
{
    ...
    {
        if (!Qs::decodeHash($st_id)) {
            return Qs::goWithDanger();
        }

        // $inv = $year ? $this->pay->getAllMyPR($st_id, $year) : $this->pay->getAllMyPR($st_id);
        $inv = $year ? $this->pay->getAllMyPR(Qs::decodeHash($st_id), $year) : $this->pay->getAllMyPR(Qs::decodeHash($st_id));

        $d['sr'] = $this->student->findByUserId(Qs::decodeHash($st_id))->first();
        // dd(Qs::decodeHash($st_id));
        // dd($d['sr']);
        $pr = $inv->get();
    ...
    public function receipts($pr_id)
    {
        ...
        try {
            $d['pr'] = $pr = $this->pay->getRecord(['id' => Qs::decodeHash($pr_id)])->with('receipt')->first();
    ...
    public function manage($class_id = NULL)
    {
        ...
        if ($class_id) {
            $d['students'] = $st = $this->student->getRecord(['my_class_id' => $class_id])->get();
            if ($st->count() < 1) {
